...
def solicitar_prestamo(id_usuario, id_herramienta, cantidad, observaciones=""):
    ...
    nuevo = {
        ...
        "fecha_devolucion_estimada": (
            hoy + timedelta(days=DIAS_PRESTAMO)
...
def aprobar_prestamo(id_prestamo):
    ...
    for prestamo in prestamos:
        if prestamo["id"] == id_prestamo:
            ...
            herramienta = mod_herramientas.buscar_herramienta(
            ...
            if herramienta["cantidad_disponible"] < prestamo["cantidad"]:
                ...
                guardar(ARCHIVO, prestamos)
                registrar_evento(
                    "RECHAZADO prestamo " + str(id_prestamo) +
                    ": se pidieron " + str(prestamo["cantidad"]) +
                    " y solo hay " +
                    str(herramienta["cantidad_disponible"])
                )
                return False
            ...
            mod_herramientas.actualizar_herramienta(
                herramienta["id"], "cantidad_disponible", nueva_cantidad
            )
            prestamo["estado"] = "activo"
            guardar(ARCHIVO, prestamos)
            registrar_evento(
                "Prestamo " + str(id_prestamo) + " aprobado"
            )
            return True
...
def devolver_prestamo(id_prestamo):
    ...
    for prestamo in prestamos:
        if prestamo["id"] == id_prestamo:
            ...
            guardar(ARCHIVO, prestamos)
            registrar_evento(
                "Prestamo " + str(id_prestamo) + " devuelto"
            )
            return True
